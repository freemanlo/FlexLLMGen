"""
Usage:
python3 -m flexllmgen.flex_llama --model meta-llama/Llama-2-7b-chat --gpu-batch-size 32 --percent 100 0 100 0 100 0
"""
...
from flexllmgen.pytorch_backend import (TorchDevice, TorchDisk, TorchLink,
    TorchMixedDevice, TorchTensor, DeviceType, general_copy, fix_recursive_import)
...
class InputEmbed:
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        # Llama doesn't use positional embeddings like OPT, just token embeddings
        ...
        embedded = F.embedding(h.data, w_token.data)
        if donate[0]: h.delete()
        hidden.val = TorchTensor.create_from_torch(embedded, self.compute)
...
class OutputEmbed:
    ...
    def load_weight(self, weight_home, weight_read_buf, k):
        w_ln, w_output = weight_home.val
        if k == 0:
            dst1 = self.weight_load_dst
            dst2 = self.compute
            weight_read_buf.store((w_ln.smart_copy(dst2),
                w_output.smart_copy(dst1)))
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if k == self.policy.num_gpu_batches - 1:
            # Clear the weight_read_buf if it is the last gpu batch
            (w_ln, donate[1]), (w_output, donate[2]) = weight_read_buf.pop()
        else:
            (w_ln, _), (w_output, _) = weight_read_buf.val

        # Llama uses RMSNorm for normalization
        # RMSNorm: x * rsqrt(mean(x^2) + eps) * weight
        import torch
        import torch.nn.functional as F
        
        # decompress weights if needed
        if w_output.device.device_type == DeviceType.COMPRESSED:
            w_output = w_output.device.decompress(w_output)
        ...
        logits = F.linear(out, w_output.data)
        last_token_logits = logits[:,-1,:]

        if self.task.do_sample and not self.task.temperature < 1e-5:
            probs = torch.softmax(last_token_logits / self.task.temperature, dim=-1)
...
class SelfAttention:
    ...
    def init_weight(self, weight_home, path):
        ...
        weight_specs = [
            # w_q
            ((h, h), dtype, layer_path + ".self_attn.q_proj.weight"),
            # b_q (Llama doesn't use bias, but backend expects it - use zeros)
            ((h,), dtype, path + "._dummy"),
            # w_k
            ((h, h), dtype, layer_path + ".self_attn.k_proj.weight"),
            # b_k
            ((h,), dtype, path + "._dummy"),
            # w_v
            ((h, h), dtype, layer_path + ".self_attn.v_proj.weight"),
            # b_v
            ((h,), dtype, path + "._dummy"),
            # w_out
            ((h, h), dtype, layer_path + ".self_attn.o_proj.weight"),
            # b_out
            ((h,), dtype, path + "._dummy"),
            # w_ln (input layer norm)
            ((h,), dtype, layer_path + ".input_layernorm.weight"),
            # b_ln (Llama uses RMSNorm with no bias)
            ((h,), dtype, path + "._dummy"),
        ]
        weights = init_weight_list(weight_specs, self.policy, self.env)
        weight_home.store(weights)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if k == self.policy.num_gpu_batches - 1:
            # Clear the weight_read_buf if it is the last gpu batch
            ((w_q, donate[2]), (b_q, donate[3]),
             (w_k, donate[4]), (b_k, donate[5]),
             (w_v, donate[6]), (b_v, donate[7]),
             (w_out, donate[8]), (b_out, donate[9]),
        ...
        if i == 0:  # prefill
            ...
            # Llama doesn't use bias terms, but backend expects them - biases are zeros
            ...
            # Llama doesn't use bias terms, but backend expects them - biases are zeros
            h, new_k_cache, new_v_cache = self.compute.mha_gen(h, mask, w_q, b_q,
                w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head,
...
class MLP:
    ...
    def init_weight(self, weight_home, path):
        ...
        weight_specs = [
            # w1
            ((f, h), dtype, path + "mlp.gate_proj.weight"),
            # w2
            ((h, f), dtype, path + "mlp.down_proj.weight"),
            # w3
            ((f, h), dtype, path + "mlp.up_proj.weight"),
            # w_ln
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if k == self.policy.num_gpu_batches - 1:
            # Clear the weight_read_buf if it is the last gpu batch
            ((w1, donate[1]), (w2, donate[2]), (w3, donate[3]),
             (w_ln, donate[4]),) = weight_read_buf.pop()
        else:
            ((w1, _), (w2, _), (w3, _),
             (w_ln, _),) = weight_read_buf.val

        # Llama MLP: RMSNorm -> gate_proj (SiLU) * up_proj -> down_proj
        import torch
        import torch.nn.functional as F
        
        # decompress weights if needed
        if w1.device.device_type == DeviceType.COMPRESSED:
            w1 = w1.device.decompress(w1)
            w2 = w2.device.decompress(w2)
            w3 = w3.device.decompress(w3)
        ...
        # Gated MLP with SiLU: down_proj(silu(gate_proj(x)) * up_proj(x))
        gate_out = F.linear(out, w1.data)
        gate_out = F.silu(gate_out)
        
        up_out = F.linear(out, w3.data)
        out = gate_out * up_out
        out = F.linear(out, w2.data)
        
        if donate[0]: h.delete()
        hidden.val = TorchTensor.create_from_torch(out, self.compute)
...
class LlamaLM:
    ...
    def init_weight(self, j):
        ...
        if DUMMY_WEIGHT not in self.path:
...
def get_filename(args):
    ...
    filename = f"fl-{model_size}-gbs{args.gpu_batch_size}-" \
...
def get_test_inputs(prompt_len, num_prompts, tokenizer):
    prompts = ["Paris is the capital city of France.",
...
def add_parser_arguments(parser):
    parser.add_argument("--model", type=str, default="meta-llama/Llama-2-7b-chat",
        help="The model name.")
